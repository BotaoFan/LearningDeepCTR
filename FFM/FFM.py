...
class FFM(BaseEstimator):
    def __init__(self, param_size, field_size=2, embedding_size=8, l2_w=0, l2_v=0,
                 learning_rate=0.001, epoch=10, batch_size=256, random_seed=42):
    ...
    def _init_weight(self):
        ...
        weight['w_linear'] = tf.Variable(tf.random_normal([self.param_size]), dtype=tf.float32, name='weight_linear')
    ...
    def _init_graph(self):
        ...
        with graph.as_default():
            ...
            self.idx = tf.placeholder(tf.int32, [None, None], 'index')
            self.val = tf.placeholder(tf.float32, [None, None], 'value')
            self.y = tf.placeholder(tf.float32, [None, 1], 'y')
            self.y_linear = tf.add(weight['w_bias'],
                                   tf.reduce_sum(
                                       tf.multiply(
                                           tf.nn.embedding_lookup(weight['w_linear'], self.idx),
                                           tf.val),
                                       1, keepdims=True)
                                   )
            self.v = []
            ...
            for i in range(self.field_size):
                self.v.append(tf.nn.embedding_lookup(weight['v'][i], tf.reshape(self.idx[:, i], [-1, 1])))
            for i in range(self.field_size):
        ...
        for epoch in range(self.epoch):
            ...
            if valid_idx is not None:
                ...
                print('[%d] Train mean loss: %.4f | Test mean loss: %.4f | Time consume is %.2f') %\
                     ((i + 1), evaluate_loss_train, evaluate_loss_test, time_consume)
            else:
                print('[%d] Train mean loss: %.4f |  Time consume is %.2f') %\
                     ((i + 1), evaluate_loss_train, time_consume)
    ...
    def predict(self, idx, val):
        n = len(idx)
        batch = int(n / self.batch_size) + 1
        y_dummy = tf.reshape([1] * n, [-1, 1])
        y_hat = None
        for i in range(batch):
            idx_batch, val_batch, y_batch = self._get_batch(idx, val, y_dummy,i)
            y_hat_batch = self.sess.run(self.y_hat,
